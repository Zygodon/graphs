...
cov <- as_adj(G0, type = "both", attr = "lor", sparse = F)
my_model <- BM_bernoulli_covariates_fast("SBM", M, cov)
my_model$estimate()
...
G0 %>% activate(edges) %>% ggraph(layout = lo) + 
  ...
  ggtitle('G0 SBM Bernouilli, cov = LOR, positive LOR') +
...
ggraph(G0, 'matrix', sort.by = NULL) + 
  ...
  ggtitle('SBM Bernouilli_cov, cov = LOR, positive LOR') +
