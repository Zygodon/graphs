...
plt <- ggplot(j1, aes(x=log(odds_ratio.x), y=log(odds_ratio.y))) +
  geom_point(aes(text = paste(from, to, sep=","))) +
  geom_errorbar(ymin = log(j1$ci_low.y), ymax = log(j1$odds_ratio.y), size = 0.1, width = 0.1, colour = "green", alpha = 0.3) +
  geom_smooth() +
  geom_abline(colour = "red") +
  labs( x = "log(Odds Ratio), assemblies", y = "log(Odds Ratio), quadrats")
plotly::ggplotly(plt)
```
Generally, odds ratios for the wider sample area (assemblies) are greater than odds ratios for the smaller sample area (quadrats) as would be expected. Blue line shows the fit. Only the 5% confidence intervals are shown; it looks as though even the outlier points are not very convincingly far from the fitted line. There are in fact no points for which ci_low(quadrats) > ci_high(assemblies) (checked with filter), and only for Carex_ovalis/Juncus_effusus is ci_low(quadrats) > odds_ratio(assemblies. Also, it is very difficult to believe that any of the identified outlying pairs have any necessary relationship with each other.

So actually not very convincing. The snark is a boojum again.
