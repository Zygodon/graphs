...
ggplot(data = G0 %>% activate(nodes), aes(deg)) + 
  geom_density(size = 1)
G1 <- G0 %>% filter(deg >= 10)

# Add in adjacent nodes
...
C <- as_adj(G1, type = "both", attr = "lor", sparse = F)
D <- as_adj(G1, type = "both", attr = "a", sparse = F)
...
community_size <- no1 %>% group_by(sbm_comm) %>% summarise(size = n())
G1 <- G1 %>% activate(nodes) %>% left_join(community_size, by = "sbm_comm")
G1 <- G1 %>% activate(nodes) %>% arrange(desc(size), sbm_comm, species)
# Re-do no1, sorted
no1 <- G1 %>% activate(nodes) %>% as_tibble(.)
# Convenient tibble
...
pal1 <- pal[as.numeric(levels(as.factor(eG1$sbm_comm)))]
...
ggraph(G1, 'matrix', sort.by = NULL) + 
  ...
  scale_y_reverse(breaks = seq(1, length(row.names(no1)), by = 1), labels = no1$species, "from") +
  scale_x_continuous(breaks = seq(1, length(row.names(no1)), by = 1), labels = no1$species, "to") +
  coord_fixed() +
  theme_bw() +
  theme(axis.text.x = element_text(size = text_size, angle = 90, colour = no1$axis_colour, 
                                   face = 'bold', hjust = 1)) +
  theme(axis.text.y = element_text(size = text_size, colour = no1$axis_colour, face = 'bold')) +
