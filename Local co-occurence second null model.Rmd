...
Here, I'm concentrating on item 3 because I had'nt thought hard enough about the null model and now think it was probably inadequate. What follows is an attempt at a better null  model which I am pretty sure will vastly reduce, and possibly eliminate entirely, all my previously supposed evidence for local pairwise association.
...
```
So now we're set up to make the modified data set, d2.
```{r echo = F, warning = F, message = F}
...
```
We want to repeat the quadrat level analysis on d2, and eventually to keep the same set of species pairs that were used to make Figure 1.
```{r echo = F, warning = F, message = F}
...
test_set <- left_join(pwor, random_q_pwor, by = c("A", "B"))
test_set <- test_set %>% select(A, B, aor, qor = qor.x, ci_low = ci_low.x, ci_high = ci_high.x, rqor = qor.y, rci_low = ci_low, rci_high = ci_high, share_2x2)

plt2 <- ggplot(test_set, aes(x = qor, y = rqor)) +
  ...
  ggtitle("Figure 2. Quadrat odds ratios: null model (ordinate) vs observed (abscissa)") +
  theme_grey() + coord_cartesian(xlim = c(-2, 3), ylim = c(-2,3))
plot(plt2)

```
Randomising the assignment of species to quadrats within stands has evidently reduced the quadrat odds ratios. The slope of the regression line in Figure 2, 0.671, is significantly less than 1.0 (t = 21 with 601 d.f, p < 0.001 ). This evidence supports the idea that there is significant local aggregation on the 2m x 2m scale. The observed and randomised quadrat odds ratios are plotted against the stand odds ratios in Figure 3.

```{r echo = F, warning = F, message = F}
# Plot observed quadrat OR ~ assembly OR, superimpose reconstructed
# quadrat OR
plt3 <- ggplot(test_set, aes(x=aor, y=qor)) +
  geom_smooth(method = lm) +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high), 
                size = 0.1, width = 0.1, colour = "green", alpha = 0.5) +
  geom_errorbar(aes(ymin = rci_low, ymax = rci_high), 
                size = 0.1, width = 0.1, colour = "grey20", alpha = 0.5) +
  geom_point(aes(x = aor, y = rqor, colour = "randomised"), alpha = 0.5) +
...
plot(plt3)
```
We are interested in species pairs that appear to be locally associated; Table 1 lists pairs for which the randomised 95% confidence limit is less than the observed 5% limit, i.e. these pairs show strong local association.

```{r echo = F, warning = F, message = F}
friends <- (test_set %>% filter(rci_high < ci_low)
...
```{r echo = F, warning = F, message = F}
enemies <- (test_set %>% filter(rci_low > ci_high)
              %>% select(A, B, qor, share_2x2)
              %>% rename("log(odds ratio)" = qor) 
              %>% rename("shared quadrats" = share_2x2))
...
Further notebooks will examine other techniques for detecting local assocation, and explore the relationships between locally associated taxa.