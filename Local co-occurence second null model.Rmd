...
```{r echo = F, warning = F, message = F}
random_q_data <- d2 %>% select(quadrat_id, species_name)
...
test_set <- test_set %>% select(aor, qor = qor.x, ci_low = ci_low.x, ci_high = ci_high.x, rqor = qor.y, rci_low = ci_low, rci_high = ci_high)
...
```
And we can see that the elaborate randomisation has had no effect whatsoever on the quadrat odds ratio.