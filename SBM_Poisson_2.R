...
# BLOCK MODEL
M <- as_adj(G0, type = "both", attr = "a", edges = FALSE, names = F, sparse = F)
my_model <- BM_poisson("SBM", M)
my_model$estimate()
model <- which.max(my_model$ICL) # Which model max ICL
...
G0 %>% activate(edges) %>% ggraph(layout = lo) + 
  ...
  ggtitle('G0 SBM Poisson, positive LOR') +
...
ggraph(G0, 'matrix', sort.by = NULL) + 
  ...
  ggtitle('SBM Poisson, positive LOR') +
